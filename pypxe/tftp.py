...
class TFTPD:
    ...
    def sendBlock(self, address):
        ...
        if len(data) != descriptor['blksize']:
            ...
            self.logger.debug('TFTP File Sent - tftp://{req_file} -> {address[0]}:{address[1]}'.format(req_file = descriptor['req_file'], address = address))
    ...
    def read(self, address, message):
        ...
        filename = self.netbootDirectory + os.sep + req_file
        if not os.path.lexists(filename):
            ...
            return
        self.ongoing[address]['req_file'] = req_file
